...
def IntensityCal(Z, kVmax, I, Ka, Kb): #defined a function to run the comparison
    loc = ('X-Ray image dataT.xls') #retrieves data for Tungsten element
    wb = xlrd.open_workbook(loc)     
    sheet = wb.sheet_by_index(0)
    xarray2 = []
    yarray2 = []
    yarray2Area = []
    for each in range(183): #append x-values to an array to be plotted as it can't be straight from the excel file
        xarray2.append(sheet.cell_value(each,0))
    for each in range(183): #append y-values to an array to be plotted as it can't be straight from the excel file
        if (kVmax-20) <= sheet.cell_value(each, 0) <= (kVmax-10): #separates values that are to be normalized and not to be normalized
    ...
    plt.plot(xarray2,yarray2) #Plots out the x-values and y-values
    ...
    for i in range(1,kVmax):
        ...
        if (Kb-1) < i < (Kb+1): #identifies the points where the Kb characteristic ray occurs and appends to the yarray
            ...
            if (kVmax-20) < i < (kVmax-10): #appends the rest of the Y-values to the yarray to be plotted
