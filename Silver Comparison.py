...
def IntensityCal(Z, kVmax, I, Ka, Kb): #defined a function to run the comparison
    loc = ('X-Ray image dataS.xls') #retrieves data for Silver element
    wb = xlrd.open_workbook(loc) #retrieves data line by line from excel 
    sheet = wb.sheet_by_index(0) 
    xarray2 = []
    yarray2 = []
    yarray2Area = []
    ...
    for each in yarrayNorm2:
        yarray2New.append(each*4)
    for each in xarray2:
        xarray2New.append(each)
    ...
    plt.plot(xarray2New,yarray2New) #Plots out the x-values and y-values
